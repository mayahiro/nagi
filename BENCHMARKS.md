...
## CLI derived Help document purpose

This benchmark validates one prebuilt Command Graph, visits every visible
command in definition-order preorder, constructs one structured Help Document
at a time, renders it, observes the output byte length, and drops it before the
next callback returns. It includes graph validation, Help construction,
escaping, and output-string allocation. It excludes graph construction,
filesystem I/O, Markdown parsing, roff formatting, shell completion, handlers,
validators, and Completion Providers

Two graph sizes and both optional formats are measured

- The one-command graph contains a root description and generated Help option
- The 1,000-command graph adds 999 visible root children, each with a
  description and note. Root Help therefore also lists all child entries
- Markdown emits CommonMark source and man emits section 1 `man` macro source
- Each run reports the sum of complete generated page bytes without retaining
  the page set

Rust reports the median of 12 complete traversals with benchmark-only total
allocation, peak additional live bytes, and retained bytes. Go reports the
median of three reports, each measured over 20 complete traversals with
standard allocation metrics. A separate Go regression renders two successive
windows and checks post-GC heap growth within a 1 MiB runtime tolerance

Run only these benchmarks from the superproject root

```sh
cargo bench --manifest-path nagi-rs/Cargo.toml -p nagi-cli-document --bench document
GOWORK="$PWD/go.work" GOTOOLCHAIN=local go test -C nagicli-go -run '^$' -bench '^BenchmarkDerivedHelpDocuments$' -benchmem -benchtime=20x -count=3 ./document
```
...
| Implementation | Format | Commands | Output bytes | Median time | Allocations | Total allocated bytes | Peak additional live bytes | Retained bytes |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Rust | Markdown | 1 | 99 | 1,125 ns | 31 | 803 | 641 | 0 |
| Rust | man | 1 | 184 | 1,042 ns | 31 | 963 | 732 | 0 |
| Rust | Markdown | 1,000 | 207,674 | 2.192 ms | 41,345 | 1,760,548 | 204,570 | 0 |
| Rust | man | 1,000 | 314,541 | 1.754 ms | 41,344 | 2,083,600 | 194,333 | 0 |
| Go | Markdown | 1 | 99 | 1,483 ns | 17 | 808 | not measured | post-GC regression only |
| Go | man | 1 | 184 | 1,419 ns | 16 | 1,096 | not measured | post-GC regression only |
| Go | Markdown | 1,000 | 207,674 | 2.564 ms | 25,067 | 2,051,654 | not measured | post-GC regression only |
| Go | man | 1,000 | 314,541 | 2.549 ms | 24,066 | 2,635,024 | not measured | post-GC regression only |

All four 1,000-command paths produce identical byte counts across Rust and Go
for a matching format and graph. Total allocation is cumulative work required
to own each structured document and returned output; it is not simultaneous
retention. Rust peak additional live memory remains below the complete output
set because traversal retains only its depth state, the current Help Document,
and the current rendered page. Every Rust sample returns to its baseline, and
the Go repeated-window test remains within its 1 MiB post-GC tolerance

The 1,000-command workload remains linear and completes below 3 ms on the
reference machine before filesystem I/O. These values are directional rather
than pass/fail thresholds. Output content, especially a wide root command,
controls both generated bytes and the largest single-page peak
